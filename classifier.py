...
# define the model
# It is composed of the EmbeddingBag layer and the linear layer
...
class TextSentiment(nn.Module):
    ...
    def init_weights(self):
        initrange = 0.5
        # uniform distribution ranging from -0.5 to 0.5
...
# There are four labels in AG_NEWS, which means that there are four classes
...
def generate_batch(batch):
    # if batch=[[0,1],[2,3],[4,5],[6,7]], label will be [0, 2, 4, 6]
    # Arrangement in the file train.csv is index_of_label /t title /t description
...
def train_func(sub_train_):
    ...
    for i, (text, offsets, cls) in enumerate(data):
        ...
        loss.backward()
        optimizer.step()
        # Whether the most closed to 1 prediction label in output equals cls
